...
export function spaceRoutes(app: FastifyInstance) {
  app.post('/api/spaces', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const space = db.prepare('SELECT * FROM spaces WHERE id = ?').get(spaceId) as any;
    const spaceCourses = db.prepare('SELECT * FROM courses WHERE space_id = ?').all(spaceId) as any[];
  ...
  app.get('/api/spaces/:id', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const courses = db.prepare('SELECT * FROM courses WHERE space_id = ?').all(id) as any[];
  ...
  app.post('/api/spaces/join', async (request, reply) => {
    ...
    const db = getDb();
    const space = db.prepare('SELECT * FROM spaces WHERE invite_code = ?').get(inviteCode) as any;
    if (!space) {
    ...
    const courses = db.prepare('SELECT * FROM courses WHERE space_id = ?').all(space.id) as any[];
