...
export function materialRoutes(app) {
    ...
    app.post('/api/courses/:id/materials', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        const result = db.prepare(`INSERT INTO materials (space_id, course_id, name, file_data, file_size, file_type, category, uploader_id)
       VALUES ((SELECT space_id FROM courses WHERE id = ?), ?, ?, ?, ?, ?, ?, ?)`).run(Number(id), Number(id), data.name, data.file_data || null, data.file_size || 0, data.file_type || 'other', data.category || 'Other', userId);
    ...
    app.get('/api/materials/:id/download', async (request, reply) => {
        const { id } = request.params;
        const db = getDb();
        const material = db.prepare('SELECT name, file_data, file_type, file_size FROM materials WHERE id = ?').get(Number(id));
