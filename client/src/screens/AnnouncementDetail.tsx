        ...
        {(!ann.attachments || ann.attachments.length === 0) && ann.file_data && ann.file_name && (
          <div className="mb-5 animate-fadeIn">
            ...
            <div className="bg-app-surface-2 rounded-2xl border border-app-border flex items-center gap-3 px-4 py-3">
              <div className="w-11 h-11 rounded-xl bg-app-accent/10 border border-app-accent/20 flex items-center justify-center text-xl flex-shrink-0">📎</div>
              <div className="flex-1 min-w-0">
                <p className="text-app-text font-jakarta font-semibold text-sm leading-snug truncate">{ann.file_name}</p>
                <p className="text-app-text-faint text-xs font-inter mt-0.5">{ann.file_size ? `${(ann.file_size / 1024).toFixed(0)} KB` : ''}</p>
              </div>
              <a href={`/api/announcements/${ann.id}/download`} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-xl bg-app-accent/10 border border-app-accent/20 flex items-center justify-center text-app-accent flex-shrink-0 hover:bg-app-accent/20 transition-colors">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                  <polyline points="7 10 12 15 17 10" />
                  <line x1="12" y1="15" x2="12" y2="3" />
